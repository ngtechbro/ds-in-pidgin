...
*
* Wetin be circular linked list?
* Circular linked list just dey like singly linked list.
* The only difference na say the TAIL no dey point to null, instead e dey point to HEAD.
* This mean say for circular linked list, HEAD = TAIL.
*
* Operations:
* - Insertion: O(1)
* - Deletion: O(1)
...
Node *createNode(int val) {
  ...
  node->next = node;  // points to itself(HEAD = TAIL).
...
* First, we go create new node with the data wey we want make the node data hold,
* then we go from HEAD, go the next node one after the other till we reach the TAIL(tail na where next pointer dey point to head) where we go add our new node.
*
* @param Node *head The head node wey dey the linked list.
* @param int data The value wey we want make our new node data hold
*
