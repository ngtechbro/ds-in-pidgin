...
*
* Doubly linked list na type of linked list wey each node wey dey the list dey hold the data wey the list go store,
* a pointer or reference to the next node, and a pointer to the previous node.
*
* Operations:
* - Insertion: O(1)
* - Deletion: O(1)
...
typedef struct Node {
  ...
  struct Node *prev;  // the pointer to the previous node for the list, or null if na the head node.
}Node;
...
* then we go set the next & prev pointer to NULL.
*
...
* NB: since this na doubly linked list, e dey possible to delete the tail and head.
*
